...
/// <summary>
/// Caching proxy for single taxonomy queries.
/// </summary>
internal sealed class CachingTaxonomyQuery : ITaxonomyQuery
{
        ...
        string codename)
    {
        ...
        _queryStateComponents.Add($"taxonomy:{codename}");
    }
    ...
    private IEnumerable<string> GetCacheDependencies(IDeliveryResult<ITaxonomyGroup> result)
    {
        if (result?.IsSuccess == true && result.Value != null)
        {
            return CacheHelpers.GetTaxonomyDependencies(result.Value);
        }
