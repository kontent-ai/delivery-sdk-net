...
internal sealed record DeliveryLanguageListingResponse : IDeliveryLanguageListingResponse
{
    ...
    /// <inheritdoc/>
    public async Task<IDeliveryResult<IDeliveryLanguageListingResponse>?> FetchNextPageAsync(CancellationToken cancellationToken = default) => !HasNextPage || NextPageFetcher is null ? null : await NextPageFetcher(cancellationToken).ConfigureAwait(false);
}
