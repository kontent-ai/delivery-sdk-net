...
When you have a custom `ITypeProvider` registered (typically generated by the model generator), typeless queries automatically resolve items to their strongly-typed models at runtime:

```csharp
// Register your type provider
services.AddSingleton<ITypeProvider, GeneratedTypeProvider>();
services.AddDeliveryClient(options => { ... });

// Now typeless queries return runtime-typed results
...
> **Recommendation**: For production applications, always use [strongly-typed models](#working-with-strongly-typed-models). They provide compile-time safety, IntelliSense support, and better maintainability.
